...
def rho(n):
    ...
    for i in range(0,pow(2,128)):
        a.append((sm3enc(message))[:int(n)])
        message=str(2*int(message)+1)
        if(sm3enc(message)[:int(n)] in a):
            print("SUCCESS！")
            b=a.index((sm3enc(message))[:int(n)])
            print("当前消息为:", message)
            print("当前消息的hash值为:", sm3enc(message))
            time_end = time.time()
            for j in range(0, b):

                message1 = str(2 * int(message1) + 1)
            print("碰撞消息为:", message1)
            print("碰撞消息的散列值为:",sm3enc(message1))
            print(b)

            print(4 * n, '比特攻击用时', time_end - time_start, 's')
            break


if __name__ == '__main__':
    n=int(input("请输入长度(单位一个字)："))
    rho(n)
